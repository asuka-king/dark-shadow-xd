...
let handler = async (m, { args, usedPrefix, command }) => {
    ...
    if (!args[0]) {
        return m.reply('Username and Repository name is missing. Example: asukakign DARK-SHADOW-XD');
    }

    // Check if the repository (args[1]) is provided
    if (!args[1]) {
        return m.reply('Repository name is missing. Example: Dark-Shadow-xd');
    }
